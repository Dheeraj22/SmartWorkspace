...
IotMqttSubscription_t subscribeInfo =
{
		...
		.pTopicFilter = MQTT_USER_PREFS,
		.topicFilterLength = (sizeof(MQTT_USER_PREFS) - 1),
		...
		IotMqttCallbackParam_t *pPublishInfo)
{
	...
	cJSON* user_data;									    // Root of the JSON data */
	cJSON* lightItem;										// Pointer to the light data */
	cJSON* fanItem;											// Pointer to the fan data */
	cJSON* overrideItem;									// Pointer to the Manual Override flag */
	cJSON* nameItem;										// Pointer to the user name */

	/*
		Scan the MQTT message for JSON data
	 */
	int num_bytes = strlen(pPayload);

	while( JSON_START != *pPayload )
	{
	...
	user_data = cJSON_Parse( pPayload );
	CY_ASSERT( NULL != user_data );

	/* Extract pointer to the user preferences */
	lightItem  = cJSON_GetObjectItemCaseSensitive(user_data, "light" );
	fanItem = cJSON_GetObjectItemCaseSensitive(user_data, "fan" );
	overrideItem = cJSON_GetObjectItemCaseSensitive(user_data, "override" );
	nameItem = cJSON_GetObjectItemCaseSensitive(user_data, "name" );
	...
	if(lightItem != NULL && fanItem != NULL && overrideItem != NULL && nameItem != NULL){

		overrideItem->valueint ? (prefs.isManualOverrideEnabled = true) : (prefs.isManualOverrideEnabled = false);
		prefs.light_duty_cycle = lightItem->valueint;
		prefs.fan_duty_cycle = fanItem->valueint;
		prefs.user_name = nameItem->valuestring;

		/* Set the common structure */
		common_set_user_prefs(&prefs);
	}
	else
	{
	...
	/* Notify the subscriber task about the received light and fan duty cycles. */
	xTaskNotify(subscriber_task_handle, 0, eSetValueWithoutOverwrite);
}
